...
def process_realcubes(x, out_size_x, out_size_y, out_size_z):
...
def indicatorize(facies_cube, codes):
    ind_cubes = np.zeros([facies_cube.shape[0], len(codes), facies_cube.shape[2], facies_cube.shape[3], facies_cube.shape[4]], np.float32)
    for i, code in enumerate(codes):
        ind_cubes[:, i:i+1] = np.where(facies_cube == code, 1., 0.)
    return ind_cubes
   
def get_grid_images(ind_cubes, grid_column):
    s = ind_cubes.shape    
    ind_cubes = ind_cubes.reshape(-1, grid_column, s[1], s[2],s[3],s[4]) # shape of [2, 9, 3, 128, 128, 32]
    ind_cubes = np.transpose(ind_cubes, axes=[0, 2, 1, 3, 4, 5])
    ind_cubes = ind_cubes.reshape(-1, 1, s[2],s[3],s[4])
    grid_reals = ind_cubes[:,:,:,:, int(s[-1] / 2)]  
    return grid_reals
...
def train_progressive_gan(
    ...
    G_repeats               = 1,            # How many times the generator is trained per G iteration.
    ...
    for gpu in range(config.num_gpus):
        with tf.name_scope('GPU%d' % gpu), tf.device('/gpu:%d' % gpu):
            ...
            reals_gpu = process_realcubes(reals_split[gpu], training_set.shape[1], training_set.shape[2], training_set.shape[3])
    ...
    grid_realcubes_inds = indicatorize(grid_realcubes, facies_codes)  # get the indicators of real facies cubes, shape of [18, len(facies_codes), 128, 128, 32]
    grid_reals = get_grid_images(grid_realcubes_inds, 9)
    ...
    grid_fakes = get_grid_images(grid_fakecubes, 9)
    misc.save_image_grid(grid_fakes, os.path.join(result_subdir, 'fakes%06d.png' % 0), drange=[0,1], grid_size=grid_size)
    if cond_well:      
        if facies_indic:
            grid_wellfacies = indicatorize(grid_wellfaciescube_process[:, 1:2], facies_codes) * grid_wellfaciescube_process[:, 0:1]  # shape of [N, 3, 128, 128, 32]
    ...
    if cond_prob:
        if facies_indic:
            grid_probs = np.concatenate([1. - np.sum(grid_probcubes, axis = 1, keepdims = True), grid_probcubes], axis = 1) # shape of [N, 3, 128, 128, 32]
            ...
            misc.save_image_grid(grid_probcubes[:,1,:,:,8], os.path.join(result_subdir, 'probimages.png'), drange=[0,1], grid_size=grid_size)  
